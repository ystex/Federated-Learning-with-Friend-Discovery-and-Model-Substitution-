...
for num in numbers:
    print(f'\n############################The Iteration {num} ############################ \n')
    if not os.path.exists('pd_cifar_%d' % lam):
        os.mkdir('pd_cifar_%d' % lam)
        # define paths
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        print('cccccccccccccccccccccccccccccclength', len(local_losses))
        ...
        df.to_csv('pd_cifar_%d/iteration_%d.csv' % (lam, num), sep=',', index=False)
