...
from comr import threshold_all, friend_cal, communication_r_ij


#1/28/2022 This is for BPC cifar10 datasets
...
for num in numbers:
    print(f'\n############################The Iteration {num} ############################ \n')
    if not os.path.exists('55cifar_bpcp_%d' % lam):
        os.mkdir('55cifar_bpcp_%d' % lam)
        # define paths
    ...
    threshold = threshold_all(args.epochs)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        #print('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', active_client)
        ...
        if epoch > 0:
            f_i_j = friend_matrix_all[-1]
            R_ij = cosine_similarity_matrix(local_weights, begin_weight, client_state, f_i_j)
            com_round = communication_r_ij(f_i_j, client_state, active_client_all)
            print('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', com_round)
        else:
            f_i_j = [[1] * m] * m
            R_ij = cosine_similarity_matrix(local_weights, begin_weight, client_state, f_i_j)
        threshold_ite = threshold[epoch]
        if epoch > 0:
            # This is com(T-1)
            ...
            M_ij = CSM_Update(M_ij_pre, R_ij, T_e_pre, client_state, f_i_j)
            M_ij_all.append(copy.deepcopy(M_ij))
            friend_matrix = friend_cal(M_ij, threshold_ite)
            friend_matrix_all.append(copy.deepcopy(friend_matrix))
        else:
            T_e = [[1] * m] * m
            communication_matrix.append(copy.deepcopy(T_e))
            M_ij_all.append(copy.deepcopy(R_ij))
            friend_matrix = friend_cal(R_ij, threshold_ite)
            friend_matrix_all.append(copy.deepcopy(friend_matrix))
        ...
        df.to_csv('55cifar_bpcp_%d/iteration_%d.csv' % (lam, num), sep=',', index=False)
