...
def cosine_similarity_matrix(weights, begin_weight, client_state, f_i_j):
    client_state_all = [item for sublist in client_state for item in sublist]
    print('1111111111111111111111', client_state_all)
...
#all set
def CSM_Update(M_ij_pre, R_ij, T_e_pre, client_state, f_i_j):
    client_state_all = [item for sublist in client_state for item in sublist]
    ...
    #print('xxxxxxxxxxxxxxxxxxxxxxx', T_e_pre)
    #print('yyyyyyyyyyyyyyyyyyyyyyyy', f_i_j)
    #print('zzzzzzzzzzzzzz', f_i_j)
    #print('ooooo', client_state_all)
    M_ij_all = []
    ir = 0
    jude_all = copy.deepcopy(0)
    for i in range(0, len(T_e_pre)):
        friend_of_i = copy.deepcopy(f_i_j[i])
        #print('zzzzzzzzzzzzzz1111111', friend_of_i)
        jr = 0
        M_ij_client = []
        for j in range(0, len(T_e_pre[0])):
            #print('uuuuuuuuuuuuu', f_i_j[i][j])
        ...
        if client_state_all[i] == 0:
            ...
            ir = ir + 1
    print('00000000000000000000xxxxxxxxxxxx', (jude_all - 10))
    return M_ij_all
